...

{
  ...
  // ==========================================
  // Group Cloud Data Sync API (0% Account Registration)
  // ==========================================
  ...
  const groupStoreMap = new Map<string, GroupStorageRecord>();

  // Fetch Group Spots & Folders
  app.get('/api/group/data', (req, res) => {
    const groupCode = (req.query.groupCode as string) || 'our-gourmet-group';
    const record = groupStoreMap.get(groupCode) || {
      spots: [],
      folders: [],
    ...
    return res.json({
      ...
      members: Array.from(record.members),
      updatedAt: record.updatedAt,
    });
  });

  // Sync / Add / Update Group Spots & Folders
  app.post('/api/group/data/sync', (req, res) => {
    const { groupCode = 'our-gourmet-group', spots = [], folders = [], memberName } = req.body;

    let record = groupStoreMap.get(groupCode);
    if (!record) {
      record = {
        spots: [],
        folders: [],
      ...
      groupStoreMap.set(groupCode, record);
    }

    if (memberName && typeof memberName === 'string') {
      record.members.add(memberName.trim());
    }

    // Merge spots by spot.id
    if (Array.isArray(spots) && spots.length > 0) {
      const spotMap = new Map<string, any>();
      // Existing spots
      for (const s of record.spots) {
        if (s && s.id) spotMap.set(s.id, s);
      }
      // Incoming spots (overwrite or append)
      for (const s of spots) {
        if (s && s.id) {
          const existing = spotMap.get(s.id);
          if (existing) {
            // Merge reactions and preserves properties
            const mergedReactions = { ...(existing.reactions || {}), ...(s.reactions || {}) };
            spotMap.set(s.id, { ...existing, ...s, reactions: mergedReactions });
          } else {
            spotMap.set(s.id, s);
          }
        }
      }
      record.spots = Array.from(spotMap.values());
    }
    ...
    if (Array.isArray(folders) && folders.length > 0) {
      ...
      record.folders = Array.from(folderMap.values());
    }
  ...
  // Delete Spot from Group
  app.post('/api/group/spot/delete', (req, res) => {
    const { groupCode = 'our-gourmet-group', spotId } = req.body;
    const record = groupStoreMap.get(groupCode);
    if (record && spotId) {
      record.spots = record.spots.filter((s) => s.id !== spotId);
      record.updatedAt = Date.now();
    }
    return res.json({
      success: true,
      spots: record ? record.spots : [],
    });
  });
